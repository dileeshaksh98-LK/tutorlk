...
function getLangLabel(lang: string): string {
  if (lang === 'SINHALA') return 'Sinhala'
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-3xl p-10 max-w-md w-full text-center">
        ...
        <div className="bg-brand-400/10 border border-brand-400/20 rounded-2xl p-4 text-left">
          ...
          <ul className="space-y-1 text-xs text-brand-400">
            <li>Reading PDF in {getLangLabel(language)}</li>
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <nav className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/admin/papers" className="text-gray-500 hover:text-white text-sm">Back to Papers</Link>
          <span className="text-gray-700">/</span>
          <span className="text-sm font-medium text-white">{step === 'upload' ? 'Upload Paper' : 'Review Questions'}</span>
        </div>
        {step === 'review' && (
          <div className="flex gap-2">
            <button onClick={() => setStep('upload')} className="px-3 py-1.5 text-sm bg-gray-800 hover:bg-gray-700 rounded-xl text-gray-300">
              Re-upload
            </button>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-5">
        {error && (
          <div className="bg-red-950/50 border border-red-800/50 text-red-300 text-sm px-4 py-3 rounded-xl">
            {error}
          </div>
        )}
          ...
          <>
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all ${
                file ? 'border-brand-400 bg-brand-400/10' : 'border-gray-700 hover:border-gray-600 hover:bg-gray-900'
              }`}
            >
              ...
              {file ? (
                <>
                  <p className="font-semibold text-brand-300 text-lg">{file.name}</p>
                  <p className="text-sm text-gray-500 mt-1">{(file.size / 1024 / 1024).toFixed(2)} MB — click to change</p>
                </>
              ) : (
                <>
                  <p className="font-semibold text-gray-300 text-lg">Click to select PDF</p>
                  <p className="text-sm text-gray-600 mt-1">Government past paper — max 15MB — Sinhala, Tamil or English</p>
                </>
              )}
            </div>
            ...
            <div className="bg-purple-950/30 border border-purple-800/30 rounded-2xl p-5 flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-white mb-1">Claude AI (Anthropic) extracts questions</h3>
                <p className="text-sm text-gray-500">
                  Reads Sinhala Unicode, Tamil Unicode and English. Detects MCQ and structured questions.
                  Maps correct answers from answer key. You review before publishing.
                </p>
              </div>
            </div>
          ...
          <>
            <div className={`rounded-xl px-4 py-3 text-sm font-medium ${
              ...
              {questions.length > 0
                ...
                : 'No questions extracted — add manually below'}
            </div>

            {questions.map((q, i) => (
              <div key={i} className={`bg-gray-900 rounded-2xl border p-5 ${
                q.type === 'MCQ' ? 'border-blue-800/30' : 'border-purple-800/30'
              }`}>
                <div className="flex items-center gap-2 mb-3">
                  <span className={`text-xs px-2.5 py-1 rounded-full font-bold ${
                    q.type === 'MCQ' ? 'bg-blue-400/20 text-blue-300' : 'bg-purple-400/20 text-purple-300'
                  }`}>{q.type}</span>
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    ...
                    <button onClick={() => removeQ(i)}
                      className="ml-1 text-red-600 hover:text-red-400 text-xs px-2 py-1 rounded hover:bg-red-950/30">
                      Remove
                    </button>
                  </div>
                </div>
                ...
                {q.type === 'MCQ' && (
                  <div className="grid sm:grid-cols-2 gap-2 mb-3">
                    {(['A', 'B', 'C', 'D'] as const).map(opt => (
                      <div key={opt} className={`flex items-center gap-2 p-2.5 rounded-xl border transition-all ${
                        q.correctOption === opt ? 'border-green-600/50 bg-green-950/30' : 'border-gray-700 hover:border-gray-600'
                      }`}>
                        <input type="radio" name={`c_${i}`} checked={q.correctOption === opt}
                          onChange={() => updateQ(i, 'correctOption', opt)}
                        ...
                        <input type="text" value={(q as any)[`option${opt}`] ?? ''}
                          onChange={e => updateQ(i, `option${opt}` as any, e.target.value)}
                          placeholder={`Option ${opt}…`}
                          className="flex-1 bg-transparent text-sm text-white focus:outline-none min-w-0 placeholder-gray-700" />
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="flex gap-3">
              <button onClick={addMCQ}
                className="px-4 py-2 text-sm border border-blue-700/50 bg-blue-950/30 text-blue-300 rounded-xl font-medium hover:bg-blue-950/50">
                Add MCQ
              </button>
              <button onClick={addStructured}
                className="px-4 py-2 text-sm border border-purple-700/50 bg-purple-950/30 text-purple-300 rounded-xl font-medium hover:bg-purple-950/50">
                Add Structured
              </button>
              <button onClick={handlePublish} disabled={!questions.length}
